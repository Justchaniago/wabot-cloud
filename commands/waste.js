...
async function getUserNickname(userId) {
    try {
        ...
        console.error('[WASTE] Nickname fetch error:', err.message);
    }
    return null;
}
...
module.exports = {
    name: 'waste',
    description: 'Input data waste harian ke spreadsheet berdasarkan grup cabang harian',
    execute: async (sock, message, args) => {
        ...
        if (!config) {
            const helpError = `
⚠️ *Akses Ditolak!*
Command \`/waste\` hanya dapat dijalankan di dalam grup resmi cabang TP atau PMS:
...
- *Grup PMS:* JID \`120363428551466980@g.us\`
        ...
        lines.shift(); // Remove /waste
        const inputText = lines.join('\n').trim();

        if (!inputText) {
            const helpText = `
⚠️ *Format /waste Salah!*
...
gt 200
        ...
        try {
            ...
            if (wasteStartIdx === -1) {
                return await sock.sendMessage(jid, { text: `❌ Header "WASTE" tidak ditemukan di kolom B pada tab "${tabName}".` });
            }
            ...
            // 5. Ask Gemini
            const prompt = `
Anda adalah AI parser tangguh untuk laporan waste/pembuangan harian toko minuman.
Tugas Anda:
...
2. Cocokkan nama item yang diketik staff ke daftar nama produk RESMI yang ada di spreadsheet di bawah kategori WASTE:
  ...
  "items": [
    ...
    { "typed": "nama_input_staff_tidak_dikenal", "matchedName": null, "quantity": angka_jumlah }
            ...
            for (const item of matchedOfficialItems) {
                const targetProd = wasteProducts.find(p => p.name.toUpperCase() === item.matchedName.toUpperCase());
            ...
            let replyText = `✅ *WASTE BERHASIL DICATAT (${config.name})*\n`;
            ...
            if (successWrites > 0) {
                ...
                replyText += `❌ *Tidak ada data waste yang berhasil dicatat.*\n`;
            }
            ...
            console.error('[WASTE] Fatal Error:', err);
