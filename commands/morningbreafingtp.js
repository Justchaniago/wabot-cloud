...
// Target Group JID for TP
const TARGET_GROUP_JID = '120363413671609227@g.us';
...
module.exports = {
    name: 'morningbreafingtp',
    aliases: ['morningbriefingtp'],
    description: 'Format & kirim Morning Briefing TP dari chat personal ke grup target',
    execute: async (sock, message, args) => {
        ...
        if (!inputText) {
            const helpText = `
⚠️ *Format Morning Briefing TP Salah!*
...
phantom PMS Hizky
        ...
        await sock.sendMessage(jid, { text: '⏳ Memproses & memparsing Morning Briefing TP...' });

        const prompt = `
Anda adalah AI parser laporan shift WhatsApp untuk toko TP6 (GC-TP6).
Tugas Anda:
...
- Format jadwal shift ("{{SCHEDULE_AREA}}"):
  ...
  - Jika ada staff Phantom (misal "phantom PMS Hizky"), buat seksi "Phantom (PMS) : Hizky". Jika tidak ada data phantom, SKIP seksi Phantom.

3. Masukkan hasil ke struktur template baku berikut:
"""
${TP_TEMPLATE_STRUCTURE}
"""
  ...
  "formattedText": "Isi lengkap template yang sudah terisi"
        ...
        for (const modelName of CANDIDATE_MODELS) {
            try {
                ...
                lastError = err;
                console.warn(`[TP_BRIEFING] Model ${modelName} failed:`, err.message);
            }
        }
        ...
        try {
            // Send pure text with mentions directly to the hardcoded Target Group JID (TP group)
            ...
            if (jid !== TARGET_GROUP_JID) {
                try {
                    await sock.sendMessage(jid, {
                        text: `✅ *Morning Briefing TP Berhasil Dikirim ke Grup TP!*\n\n*Preview Terkirim:*\n\n${formattedText}`,
                        mentions: mentionedJids
                    });
                } catch (e) {
                    console.warn('[TP_BRIEFING] Personal chat confirmation skipped:', e.message);
                }
            }
        } catch (err) {
            console.error('[TP_BRIEFING] Error sending message to group:', err);
            try {
